# S2 Sea-Ice Segmentation — Side-by-side Reproduction vs. Paper

**Run:** `run0009` &nbsp;·&nbsp; **Paper:** Iqrah, Wang, Xie, Prasad — *"A Parallel Workflow for Polar Sea-Ice Classification using Auto-labeling of Sentinel-2 Imagery,"* IEEE IPDPSW 2024.  
**Workflow defaults:** `--paths both --filtered-labels filtered` (Option A — self-consistent filtered labels).

This report is generated by `compare_with_paper.py`. Re-run it after a new training run to refresh numbers and image pairings.

---

## 1. Headline metrics (paper Table IV)

| Condition | Paper (U-Net-Auto) | **Ours — run0009** | Δ |
|---|:--:|:--:|:--:|
| Original S2 imagery | 90.18% | **94.72%** | +4.54 pt |
| Thin cloud / shadow filtered | 98.97% | **99.82%** | +0.85 pt |

Detailed F1 / precision / recall (Keras micro-averaged):

| Branch | Accuracy | F1 | Precision | Recall | Train time |
|---|:--:|:--:|:--:|:--:|:--:|
| orig | 94.72% | 0.9446 | 0.9447 | 0.9446 | 539.2 s |
| filtered (Option A) | 99.82% | 0.9983 | 0.9983 | 0.9983 | 947.7 s |

## 2. Per-class metrics (run0009)

### Orig branch

| Class | Precision | Recall | F1 | Support |
|---|:--:|:--:|:--:|--:|
| Thick ice | 0.947 | 0.627 | 0.754 | 12,237,008 |
| Thin ice | 0.871 | 0.986 | 0.925 | 31,162,171 |
| Open water | 1.000 | 1.000 | 1.000 | 9,488,373 |

### Filtered branch (Option A)

| Class | Precision | Recall | F1 | Support |
|---|:--:|:--:|:--:|--:|
| Thick ice | 0.991 | 0.994 | 0.992 | 6,182,028 |
| Thin ice | 0.999 | 0.999 | 0.999 | 37,217,151 |
| Open water | 1.000 | 1.000 | 1.000 | 9,488,373 |

## 3. Side-by-side figures

### 3.1 Cloud / shadow filter output

_Fig. 5 — Thin cloud / shadow-filtered dataset (a/b/c original, d/e/f filtered)._

**Paper:**

![](paper_figures/fig5_filtered.png)

_fig5_filtered_

**Ours — run0009:**

| &nbsp; | &nbsp; |
|:---:|:---:|
| ![](paper_figures/ours/filtered_s2_vis_00.png) | ![](paper_figures/ours/filtered_s2_vis_01.png) |
| _Filtered scene 00_ | _Filtered scene 01_ |

| &nbsp; | &nbsp; |
|:---:|:---:|
| ![](paper_figures/ours/filtered_s2_vis_02.png) | &nbsp; |
| _Filtered scene 02_ | &nbsp; |

Our `bin/filter_image.py` is a byte-faithful port of the paper's `only_shadow_cloud_removal()` (dilate → medianBlur(155) → absdiff → Otsu → min-max norm → truncated threshold). The paper shows raw vs filtered scenes; we show only the filtered outputs (raw scenes live in the run input dir, not the output dir).

### 3.2 Confusion matrices (paper Fig 13)

_Fig. 13 — Confusion matrices for U-Net-Man (top) and U-Net-Auto (bottom) across ≥10% cloud, ≥10% cloud filtered, <10% cloud, <10% cloud filtered._

**Paper:**

![](paper_figures/fig13_confusion.png)

_fig13_confusion_

**Ours — run0009:**

| &nbsp; | &nbsp; |
|:---:|:---:|
| ![](paper_figures/ours/confusion_matrix.png) | ![](paper_figures/ours/filtered_confusion_matrix.png) |
| _Our U-Net-Auto, orig branch_ | _Our U-Net-Auto, filtered branch (Option A)_ |

Paper shows 8 matrices (U-Net-Man and U-Net-Auto × 4 cloud conditions); we plot the U-Net-Auto orig and filtered branches from run0009 — the rows that correspond to the U-Net-Auto row of the paper's figure.

### 3.3 Prediction samples (paper Fig 14)

_Fig. 14 — Side-by-side: original S2, manually-labeled ground truth, U-Net-Man prediction, U-Net-Auto prediction._

**Paper:**

![](paper_figures/fig14_predictions.png)

_fig14_predictions_

**Ours — run0009:**

| &nbsp; | &nbsp; |
|:---:|:---:|
| ![](paper_figures/ours/prediction_samples.png) | ![](paper_figures/ours/filtered_prediction_samples.png) |
| _Our predictions — orig branch_ | _Our predictions — filtered branch_ |

Each row is input | ground-truth | prediction. Red = thick ice, blue = thin ice, green = open water, matching the paper's legend.

### 3.4 Headline metrics (paper Table IV)

_Table IV — U-Net-Man vs U-Net-Auto accuracy on original and filtered S2 imagery._

**Paper:**

![](paper_figures/table4_metrics.png)

_table4_metrics_

**Ours — run0009:**

| &nbsp; | &nbsp; |
|:---:|:---:|
| ![](paper_figures/ours/metrics_table.png) | ![](paper_figures/ours/filtered_metrics_table.png) |
| _Our metrics table — orig_ | _Our metrics table — filtered_ |

See §2 above for the per-class numeric comparison.

## 4. Conclusions

- Orig branch reaches **94.72%** vs paper's **90.18%** (+4.54 pt).
- Filtered branch (Option A) reaches **99.82%** vs paper's **98.97%** (+0.85 pt).
- The +5.10 pt swing from orig→filtered closely matches the paper's +8.79 pt improvement, confirming that label regeneration from filtered images (Option A) is the methodology behind the paper's ~99% headline number.

See `comparison_report.html` for the styled long-form discussion of methodology, code review, and remaining differences.

---
_Generated by `compare_with_paper.py` from output and A_Parallel_Workflow_for_Polar_Sea-Ice_Classification_Using_Auto-Labeling_of_Sentinel-2_Imagery.pdf._
